...
import { getPrizeByRank, getPrizeSettings } from '@/services/rank/prizebyrank';
import { getRoleinfo } from '@/services/rank/roleinfo';
...
const RankingPage = () => {
  ...
  const [roleinfoData, setRoleinfoData] = useState(null);
  ...
  // ฟังก์ชันหาข้อมูล roleinfo
  const fetchRoleinfo = async (roleId, serverId) => {
    try {
      const roleinfoResult = await getRoleinfo();
      setRoleinfoData(roleinfoResult.data);
      console.log('Fetched roleinfo data:', roleinfoResult.data);

      if (roleinfoResult && roleinfoResult.success) {
        const roleinfoData = roleinfoResult.data;
        const matchedInfo = roleinfoData.find(info =>
          info.roleid === roleId && info.serverid === parseInt(serverId)
        );

        return matchedInfo || null;
      }
    } catch (err) {
      console.error('Failed to fetch roleinfo:', err);
      return null;
    }
    return null;
  };
  ...
  const fetchPrizeData = async (serverId = selectedServer) => {
    ...
    try {
      const [prizeByRankResult, prizeSettingsResult] = await Promise.all([
        getPrizeByRank(),
        getPrizeSettings()
      ].map(p => p.catch(e => ({ success: false, error: e }))));

      if (prizeByRankResult?.success && prizeSettingsResult?.success) {
        const settings = prizeSettingsResult.data?.find(s => s.serverid === parseInt(serverId));
        if (!settings) {
          throw new Error('Prize settings not found for server ' + serverId);
        }

        const totalPrize = parseFloat(settings.initial_prize || '0') + parseFloat(settings.addon_prize || '0');

        // คำนวณรางวัลตาม % ที่กำหนด
        const prizeDataFromAPI = prizeByRankResult.data?.map(prize => {
          const prizeAmount = (totalPrize * parseFloat(prize.percent_prize || '0')) / 100;
          return {
            ...prize,
            server_id: serverId,
            percent_prize: prizeAmount.toFixed(2)
          };
        }) || [];
  ...
  const fetchRankingData = async () => {
    ...
    try {
      const roleInfoData = await getRoleinfo();

      if (!roleInfoData.success || !roleInfoData.data) {
        throw new Error('Failed to fetch role info data');
      }

      // กรองข้อมูลที่มี gamelevel ไม่เป็น null หรือ 0 และเรียงลำดับจากมากไปน้อย
      const sortedRoleData = roleInfoData.data
        .filter(role => role.gamelevels != null && role.gamelevels !== -1)
        .sort((a, b) => b.gamelevels - a.gamelevels);

      // แปลงข้อมูลให้อยู่ในรูปแบบที่ UI ต้องการ
      const transformedLevelData = sortedRoleData
        .filter(role => role.serverid === parseInt(selectedServer))
        .map((role, index) => ({
          rank: index + 1,
          name: role.info?.name || 'Unknown Player',
          level: role.gamelevels || 0,
          roleId: role.id || '',
          serverId: role.serverid || 0,
          class: 'Adventurer',
          guild: '-',
          power: role.gamelevels * 1000 || 0
        }));

      setRankingData(prev => ({
        ...prev,
        level: transformedLevelData,
        arena: [], // ไม่ใช้ arena ranking แล้ว
      }));
    } catch (err) {
  ...
  // Countdown Timer - วันที่ 22 พฤศจิกายน 2568 เวลา 23:59:00
  useEffect(() => {
    const calculateCountdown = () => {
      // วันที่เป้าหมาย: 22 พฤศจิกายน 2568 = 22 November 2025 (ค.ศ.)
      const endDate = new Date('2025-11-21T23:59:00+07:00'); // GMT+7 (Thailand)
      const now = new Date();
  ...
  // Navigate to rewards page with token
  const handleGoToRewards = () => {
    if (hasToken) {
      ...
      window.location.href = `/rewardss1?${params.toString()}`;
    }
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900">
      ...
      <div className="bg-slate-900/95 backdrop-blur-md border-b border-slate-700">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          >
            {/* Rewards Button - แสดงเฉพาะเมื่อมี token */}
            {hasToken && (
              <motion.div
              ...
              >
                <button
                  ...
                  className="flex items-center space-x-2 px-4 py-2 bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-500 hover:to-pink-500 text-white font-bold rounded-xl transition-all duration-300 shadow-lg hover:shadow-purple-500/50 transform hover:scale-105"
                >
                  <Gift className="w-5 h-5" />
                  <span className="hidden sm:inline">แลกของรางวัล</span>
                </button>
              </motion.div>
            )}
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <motion.div
          ...
          className="mb-6"
        >
          <div className="bg-gradient-to-r from-red-500/20 via-orange-500/20 to-yellow-500/20 backdrop-blur-sm rounded-2xl border border-orange-500/50 overflow-hidden shadow-lg shadow-orange-500/20">
            <div className="px-6 py-4">
              <div className="flex items-center justify-center flex-wrap gap-4">
                <div className="flex items-center">
                  <Flame className="w-6 h-6 text-orange-400 mr-2 animate-pulse" />
                  <span className="text-white font-bold text-lg">สิ้นสุดการแข่งขัน Season นี้:</span>
                </div>
                <div className="flex items-center bg-slate-900/50 px-4 py-2 rounded-xl border border-orange-500/30">
              ...
              <div className="text-center mt-4">
                <div className="inline-flex items-center justify-center gap-3 bg-slate-900/70 px-6 py-3 rounded-xl border border-yellow-500/30">
                  ...
                  <div className="flex items-center gap-2">
                    <div className="text-center">
                      ...
                      <div className="text-xs text-gray-300">วัน</div>
                    </div>
                    ...
                    <div className="text-center">
                      ...
                      <div className="text-xs text-gray-300">ชม.</div>
                    </div>
                    <span className="text-2xl text-gray-400 font-bold">:</span>
                    <div className="text-center">
              ...
              <div className="text-center mt-3">
                <p className="text-sm text-orange-200">
                  ⏰ รีบทำคะแนนให้ได้อันดับสูงสุดเพื่อรับรางวัลใหญ่!
                </p>
              </div>
            </div>
          </div>
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center space-x-2">
                  {/* <div className='mt-2'>
                    <Link
                      href="/rewardss1"
                      className="inline-flex items-center space-x-2 px-4 py-2 bg-gradient-to-r from-yellow-500/20 to-orange-500/20 hover:from-yellow-500/30 hover:to-orange-500/30 border border-yellow-500/30 rounded-lg text-yellow-400 hover:text-yellow-300 transition-all duration-300 "
                    >
                      <span>ดูรางวัล</span>
                    ...
                    <div className="flex items-end justify-center space-x-4 max-w-2xl mx-auto">
                      ...
                      {prizeData.find(p => p.from_rank === 2) && (
                        ...
                        >
                          ...
                          <div className="mt-2 text-center">
                            <div className="text-sm font-bold text-white">
                              {rankingData.level.find(player => player.rank === 2)?.name || 'รอผู้เล่น'}
                            </div>
                      ...
                      {prizeData.find(p => p.from_rank === 1) && (
                        ...
                        >
                          ...
                          <div className="mt-2 text-center">
                            <div className="text-sm font-bold text-white">
                              {rankingData.level.find(player => player.rank === 1)?.name || 'รอผู้เล่น'}
                            </div>
                      ...
                      {prizeData.find(p => p.from_rank === 3) && (
                        ...
                        >
                          ...
                          <div className="mt-2 text-center">
                            <div className="text-sm font-bold text-white">
                              {rankingData.level.find(player => player.rank === 3)?.name || 'รอผู้เล่น'}
                            </div>
                          ...
                          <div className="flex items-center space-x-3">
                            <div className="text-right">
                              <div className="text-lg font-bold bg-gradient-to-r from-yellow-400 to-orange-500 bg-clip-text text-transparent">
                                {parseFloat(prize.percent_prize).toLocaleString()} Token
                              </div>
          ...
          <div className="bg-slate-800/50 rounded-xl p-1 flex space-x-1">
            {[
              ...
              <button
                ...
                disabled={loading}
                className={`flex items-center space-x-2 px-6 py-3 rounded-lg font-medium transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed ${activeTab === tab.id
                ...
                <table className="w-full">
                  <thead className="bg-slate-800/50">
                    <tr>
                      ...
                      {activeTab === 'level' && (
                        <>
                          <th className="px-6 py-4 text-left text-sm font-medium text-gray-300">ด่าน</th>
